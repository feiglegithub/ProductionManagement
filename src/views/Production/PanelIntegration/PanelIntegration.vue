        ...
        <div class="m-inp f-mtb5">
          ...
          <span class="inp s-inpbg" style="width: 65%">
            <input
              ...
              placeholder="请扫描UPI"
              type="text"
              @keyup.enter="getSourceNumber()"
              class="s-inpbg"
            />
            <span
              class="iconfont icon-iconfontscan"
              @click="scanBarCodeByUpi"
            ></span>
          </span>
        </div>
...
export default {
  ...
  methods: {
    ...
    scanBarCodeByUpi() {
      ...
      this.$hybridApi.scanCode({
        success: function (res) {
          that.SourceNumber = res;
          that.getSourceNumber();
        },
      });
    },
    //改变切换按钮
    ...
    getSupportNumber() {
      ...
      if (!!this.SupportNumber && !!this.SourceNumber) {
        if (this.IsProduction == "是") {
          this.checkWorkPlateJustChangePlateCodePda(
            this.SupportNumber,
            this.SourceNumber
          );
        }
        if (this.IsProduction == "否") {
          this.checkWorkPlateMixableByPlate(
            this.SupportNumber,
            this.SourceNumber
          );
        }
    ...
    doPost() {
      ...
      if (this.IsProduction == "是") {
        console.log("是");
        this.doChangeWorkPlateJustChangePlateCodePda(
          this.PostSupportNumber,
          this.PostSourceNumber
        );
      }
      if (this.IsProduction == "否") {
        console.log("否");
        this.doChangeWorkPlateByPlatePda(
          this.PostSupportNumber,
          this.PostSourceNumber
        );
      }
    ...
    checkWorkPlateJustChangePlateCodePda(TargetBasePlateNo, SourceBasePlateNo) {
      ...
      this.showThost = true;
      this.$axiosApi
        .checkWorkPlateJustChangePlateCodePda(
          TargetBasePlateNo,
          SourceBasePlateNo
        )
        .then((res) => {
    ...
    //接口：提交转托动作（生产中）
    doChangeWorkPlateJustChangePlateCodePda(
      TargetBasePlateNo,
      SourceBasePlateNo
    ) {
      this.loadingtitle = "提交中";
