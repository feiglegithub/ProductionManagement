<template>
  <div class="g-index">
    ...
    <div class="g-flexsb2">
      <el-button class="btn" @click="doPost">修改责任班组</el-button>
      <el-button class="btn" @click="submitOwn">确认补料</el-button>
      <el-button class="btn" @click="submitOwnRecovery">欠件找回</el-button>
      <el-button class="btn" @click="submitOwnRevoke">欠件撤销</el-button>
    </div>
    <div class="f-flexvw f-flexg1 f-pdlr5" style="overflow-y: auto">
      <div class="g-scrollbox">
        <div class="f-auto">
          ...
          >
            <div
              class="m-listbox f-mtb5 s-bgE7E7E7"
              v-for="(item, index) in DataList"
              :key="index"
              @click="clickChoice(item, index)"
            >
              <div class="btnNum">
                <el-checkbox-button v-model="item.IsSelect" border>
              ...
              <div class="showbox">
                ...
                <div
                  class="m-baserowbox"
                  v-if="item.ResponseData.ResRemark != '非生产性责任班组'"
                >
                  <span class="label80">责任人:</span>
                  <div
                    class="select s-bgwhile"
                    @click="clickPersonLiable(item, index)"
                  >
                ...
                <div class="m-baserowbox">
                  ...
                  <div
                    class="select s-bgwhile"
                    @click="clickRelationPerson(item, index)"
                  >
...
export default {
  ...
  methods: {
    ...
    doPost() {
      ...
      this.ownModifyWorkGroup(this.FeedingReworkData);
    },
    ...
    submitOwn() {
      if (this.IsSelectItem()) {
        this.showThost = true;
        this.$axiosApi
          .submitOwnReproduce(this.FeedingReworkData)
          .then((res) => {
    ...
    submitOwnRecovery() {
      if (this.IsSelectItem()) {
        this.showThost = true;
        this.$axiosApi
          .submitOwnRecovery(this.FeedingReworkData.Details)
          .then((res) => {
            this.showThost = false;
            console.log(res);
            if (res.Success == true) {
    ...
    submitOwnRevoke() {
      if (this.IsSelectItem()) {
        this.showThost = true;
        this.$axiosApi
          .submitOwnRevoke(this.FeedingReworkData.Details)
          .then((res) => {
            this.showThost = false;
            console.log(res);
            if (res.Success == true) {
    ...
    IsSelectItem() {
      let isSelect = false;
      this.FeedingReworkData.Details.forEach((item) => {
        if (item.IsSelect) isSelect = true;
      });
      if (!isSelect) {
        this.showPositionValue = true;
        this.Msg = "请选择明细行，再进行操作！";
        return false;
      }
      return true;
    },
    ...
    changeGroup(val) {
      ...
      this.DataList[myindex].ResponseData.ResWorkGroup = this.GetGroup.find(
        (item) => item.Id == id
      ).Name;
    },
    //选择的责任人
    changePersonLiable(val) {
      ...
      if (!!this.DataList[myindex].ResponseData.ResEmployeeId) {
        this.DataList[
          myindex
        ].ResponseData.ResEmployee = this.GetPersonLiable.find(
          (item) => item.Id == id
        ).Name;
      } else {
        this.DataList[myindex].ResponseData.ResEmployeeId = null;
    ...
    changeLiableExamine(val) {
      ...
      if (
        this.DataList[myindex].ResponseData.ResEmpAssessmentName == "已考核"
      ) {
        this.DataList[myindex].ResponseData.ResEmpAssessment = 1;
      }
      if (
        this.DataList[myindex].ResponseData.ResEmpAssessmentName == "未考核"
      ) {
    ...
    changeRelationPerson(val) {
      ...
      if (!!this.DataList[myindex].ResponseData.JointEmpId) {
        this.DataList[
          myindex
        ].ResponseData.JointEmp = this.GetRelationPerson.find(
          (item) => item.Id == id
        ).Name;
      } else {
        this.DataList[myindex].ResponseData.JointEmpId = null;
    ...
    changeRelationExamine(val) {
      ...
      if (
        this.DataList[myindex].ResponseData.JonitEmpAssessmentName == "已考核"
      ) {
        this.DataList[myindex].ResponseData.JonitEmpAssessment = 1;
      }
      if (
        this.DataList[myindex].ResponseData.JonitEmpAssessmentName == "未考核"
      ) {
    ...
    clickGroup() {
      ...
      this.$axiosApi
        .getRepWorkGroups(
          this.FeedingReworkData.DeptId,
          this.DataList[myindex].ResponseData.ResRemark
        )
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    clickPersonLiable(item, index) {
      ...
      //再执行接口调取
      if (
        this.DataList[myindex].ResWorkGroupId === null ||
        this.DataList[myindex].ResWorkGroupId === ""
      ) {
        this.showPositionValue = true;
        this.Msg = "请先选择责任班组";
        return;
      ...
      this.$axiosApi
        .getRepEmps(0, this.DataList[myindex].ResponseData.ResWorkGroupId)
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    clickRelationPerson(item, index) {
      ...
      if (
        this.DataList[myindex].ResponseData.ResWorkGroupId === null ||
        this.DataList[myindex].ResponseData.ResWorkGroupId === ""
      ) {
        this.showPositionValue = true;
        this.Msg = "请先选择责任班组";
        return;
      ...
      // 0-选择责任人，1-选择连带责任人，2-选择定责质检
      this.$axiosApi
        .getRepEmps(1, this.DataList[myindex].ResponseData.ResWorkGroupId)
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    //接口：修改责任班组
    ownModifyWorkGroup(data) {
      this.loadingtitle = "提交中";
      this.showThost = true;
      this.$axiosApi.ownModifyWorkGroup(this.FeedingReworkData.Details).then((res) => {
        this.showThost = false;
        console.log(res);
        if (res.Success == true) {
          this.ConfirmMsg = "提交成功";
