        ...
        <div class="f-flexvw f-flexg1">
            <div class="g-scrollbox">
                <div class="m-inp f-mtb5">
                    ...
                    <div class="m-selector" @click="ShowWorkshop = true">
                ...
                <div class="m-inp f-mtb5">
                    ...
                    <div class="m-selector" @click="ShowProcess = true">
                        <popup-picker :show.sync="ShowProcess" :data="ProcessList" @on-change="changeProcess" value-text-align='left'></popup-picker>
                        <div>{{ChoiceProcess}}</div>
                    </div>
                </div>
                <div class="m-inp f-mtb5">
                    <span class="laber">资源</span>
                    <div class="m-selector" @click="doShowResource">
                        <popup-picker :show.sync="ShowResource" :data="ResourceList" @on-change="changeResource" value-text-align='left'></popup-picker>
                        <div>{{ChoiceResource}}</div>
                    </div>
                </div>
                <div class="m-selectinp f-mtb5" style="justify-content: space-around;">
                    <div :class="{ activestyle: !IsLaminate }" style="font-size: 16px; margin-left: 5px;">&nbsp;&nbsp;正单&nbsp;&nbsp;</div>
                    <s-switch @changeSwitch='IsLaminate = !IsLaminate' :inputValue='IsLaminate'></s-switch>
                    <div :class="{ activestyle: IsLaminate }" style="font-size: 16px; margin-left: 5px;">标准层板</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        goReturnRepair() {
            ...
            this.checkResourceAttendance(this.ChoiceWorkshopId, this.ChoiceProcessId, this.ChoiceResourceId)
        },
        //选择某一项车间
        changeWorkshop(value) {
            let id = value[0]
            this.ChoiceWorkshopId = value[0]
            localStorage.setItem('ReturnRepairWorkshopKey', this.ChoiceWorkshopId)
        ...
        changeProcess(value) {
            ...
            localStorage.setItem('ReturnRepairProcessKey', this.ChoiceProcessId)
            this.ChoiceProcess = this.GetProcess.find(item => item.Id == id).Name
            this.ChoiceResourceId = null
            this.ChoiceResource = null
            localStorage.setItem('ReturnRepairResourceKey', this.ChoiceResourceId)
        },
        //选择某一项资源
        ...
        //接口：查找考勤情况
        checkResourceAttendance(workshopId, processId, resourceId) {
            this.LoadingTitle = '加载中'
            this.ShowThost = true
            this.$axiosApi.packageCheckAttendance(workshopId, processId, resourceId).then(res => {
                ...
                if (res.Success) {
                    // console.log(res);
                    this.$router.push({
                        name: this.IsLaminate ? 'ReturnRepairBatch' : 'ReturnRepairUpi',
                        params: {
        ...
        async judgeWorkshop() {
            ...
            if (localStorage.getItem('ReturnRepairWorkshopKey')) {
                let newArr = this.GetWorkshop.filter(item => {
                    return item.Id == localStorage.getItem('ReturnRepairWorkshopKey')
                })

                if (newArr.length > 0) {
                    this.ChoiceWorkshopId = localStorage.getItem('ReturnRepairWorkshopKey')
        ...
        async judgeProcess() {
            ...
            if (localStorage.getItem('ReturnRepairProcessKey')) {
                let newArr = this.GetProcess.filter(item => {
                    return item.Id == localStorage.getItem('ReturnRepairProcessKey')
                })

                if (newArr.length > 0) {
                    this.ChoiceProcessId = localStorage.getItem('ReturnRepairProcessKey')
        ...
        async judgeResources() {
            ...
            if (localStorage.getItem('ReturnRepairResourceKey')) {
                let newArr = this.GetResource.filter(item => {
                    return item.Id == localStorage.getItem('ReturnRepairResourceKey')
                })

                if (newArr.length > 0) {
                    this.ChoiceResourceId = localStorage.getItem('ReturnRepairResourceKey')
