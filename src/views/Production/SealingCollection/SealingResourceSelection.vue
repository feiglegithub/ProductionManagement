...
export default {
    ...
    methods: {
        ...
        changeWorkshop(value){
            ...
            localStorage.setItem('SealingWorkshopKey',this.ChoiceWorkshopId)
        ...
        changeProcess(value){
            ...
            localStorage.setItem('SealingProcessKey',this.ChoiceProcessId)
            this.ChoiceProcess = this.GetProcess.find(item=>item.Id == id).Name
            this.ChoiceResourceId=null
            this.ChoiceResource=null
            localStorage.setItem('SealingResourceKey',this.ChoiceResourceId)
        },
        //选择某一项资源
        ...
        async judgeWorkshop(){
            ...
            if (localStorage.getItem('SealingWorkshopKey')) {
                let newArr=this.GetWorkshop.filter(item=>{
                    return item.Id==localStorage.getItem('SealingWorkshopKey')
                })
                if(newArr.length>0){
                    this.ChoiceWorkshopId=localStorage.getItem('SealingWorkshopKey')
        ...
        async judgeProcess(){
            ...
            if (localStorage.getItem('SealingProcessKey')) {
                let newArr=this.GetProcess.filter(item=>{
                    return item.Id==localStorage.getItem('SealingProcessKey')
                })
                if(newArr.length>0){
                    this.ChoiceProcessId=localStorage.getItem('SealingProcessKey')
        ...
        async judgeResources(){
            ...
            if (localStorage.getItem('SealingResourceKey')) {
                let newArr=this.GetResource.filter(item=>{
                    return item.Id==localStorage.getItem('SealingResourceKey')
                })
                if(newArr.length>0){
                    this.ChoiceResourceId=localStorage.getItem('SealingResourceKey')
