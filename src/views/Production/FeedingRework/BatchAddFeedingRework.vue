              ...
              >
                ...
                >
                  <div slot="right-menu" style="padding: 5px 0">
                    <swipeout-button
                      @click.native="onButtonClick(index, idx, item)"
                      type="warn"
                      >删除</swipeout-button
                    >
                  </div>
  ...
  methods: {
    ...
    handleTabsEdit(targetName, action) {
      if (action === "add") {
        ...
        this.$axiosApi.getCreateBatchFeedingObj(newTabName).then((res) => {
          if (res.Success == true) {
            console.log(res);
            this.FeedingReworkData.PcDetails.push(res.Result);
            this.SelIndex = this.tabIndex - 1;
          } else {
        ...
        this.editableTabs.push({
          title: "出错" + this.tabIndex,
          name: newTabName,
          content: "",
        });
        this.editableTabsValue = newTabName;
        this.SelTabName = newTabName;
      }
      if (action === "remove") {
    ...
    doTemporaryStorage() {
      ...
      if (this.FeedingReworkData.IsBatch == 1) {
        ...
        this.FeedingReworkData.PhotoList = this.arrayImage;
        if (
          this.FeedingReworkData.PhotoList == null ||
          this.FeedingReworkData.PhotoList == ""
        ) {
          this.FeedingReworkData.PhotoList = [];
        }
    ...
    doPost() {
      ...
      if (this.FeedingReworkData.IsBatch == 1) {
        ...
        this.FeedingReworkData.PcDetails.forEach((item) => {
          ...
          if (!item.rData.ResRemark) {
            this.showPositionValue = true;
            this.Msg = `责任班组标识不能为空`;
            return;
          }
          if (!item.rData.ResWorkGroupId) {
            this.showPositionValue = true;
            this.Msg = `责任班组不能为空`;
            return;
          }
          if (
            item.rData.ResRemark == "非生产性责任班组" &&
            item.rData.QualityInspection == null
          ) {
    ...
    changeGroupType($event, index) {
      ...
      if (
        ...
        this.FeedingReworkData.PcDetails[index].rData.ResWorkGroup = null;
        this.FeedingReworkData.PcDetails[index].rData.ResWorkGroupId = null;
    ...
    changeGroup($event, index) {
      ...
      //重置与班组关联的查询信息
      if (
        this.FeedingReworkData.PcDetails[index].rData.ResWorkGroupId !=
        $event[0]
      ) {
        this.FeedingReworkData.PcDetails[index].rData.ResEmployee = null;
        this.FeedingReworkData.PcDetails[index].rData.ResEmployeeId = null;
    ...
    //选择的责任人
    changePersonLiable($event, index) {
      let id = $event[0];
      index = this.SelIndex;
    ...
    changeLiableExamine($event, index) {
      ...
      this.FeedingReworkData.PcDetails[index].rData.StrResEmpAssessment =
        $event[0];
      if ($event[0] == "已考核") {
        this.FeedingReworkData.PcDetails[index].rData.ResEmpAssessment = 1;
      }
      if ($event[0] == "未考核") {
        this.FeedingReworkData.PcDetails[index].rData.ResEmpAssessment = 0;
      }
    },
    //选择连带责任人
    changeRelationPerson($event, index) {
      let id = $event[0];
      index = this.SelIndex;
    ...
    changeRelationExamine($event, index) {
      ...
      this.FeedingReworkData.PcDetails[index].rData.StrJonitEmpAssessment =
        $event[0];
      if ($event[0] == "已考核") {
        this.FeedingReworkData.PcDetails[index].rData.JonitEmpAssessment = 1;
      }
      if ($event[0] == "未考核") {
        this.FeedingReworkData.PcDetails[index].rData.JonitEmpAssessment = 0;
      }
    },
    //选择制单人
    changeMaker(val) {
    ...
    clickEquipment() {
      ...
      this.$axiosApi
        .getRepResourceManages(this.FeedingReworkData.DeptId)
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    clickGroup() {
      let index = this.SelIndex;
      this.ShowGroup = true;
      if (!this.FeedingReworkData.PcDetails[index].rData.ResRemark) {
        this.showPositionValue = true;
        this.Msg = "请先选择责任班组类别";
        return;
      }
      this.$axiosApi
        .getRepWorkGroups(
          ...
          this.FeedingReworkData.PcDetails[index].rData.ResRemark
        )
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    clickQualityTest() {
      ...
      this.$axiosApi
        .getDetermineEmps(this.FeedingReworkData.ProduceTaskId)
        .then((res) => {
          if (res.Success == true) {
            console.log(res);
    ...
    InitTemporaryStorage() {
      let num = this.FeedingReworkData.PcDetails.length;
      for (let i = 0; i < num; i++) {
        this.consoleAddTab();
      }
      this.SelTabName = this.editableTabsValue;
      this.SelIndex = num - 1;
    },
      ...
      this.$axiosApi
        ...
        .then((res) => {
          ...
          if (res.Success == true) {
            ...
            if (
              this.NowBatchNo != null &&
              res.Result.BatchNo != this.NowBatchNo
            ) {
              console.log(res.Result.BatchNo);
              this.showPositionValue = true;
              this.Msg = "不同批次都不能添加";
    ...
    //接口：删除补料返工单明细
    deleteProduceTaskDetail(upi) {
      this.loadingtitle = "删除中";
      this.showThost = true;
      let that = this;
      this.$axiosApi.deleteProduceTaskDetail(upi).then((res) => {
        this.showThost = false;
        if (res.Success == true) {
          console.log(res);
          this.showPositionValue = true;
