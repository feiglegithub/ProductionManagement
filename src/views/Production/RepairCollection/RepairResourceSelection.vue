...
export default {
    ...
    methods: {
        ...
        //点击提交按钮
        goCollection(){
            this.$router.push({name:'RepairCollection'})
        },
        changeWorkshop(value){
            this.ChoiceWorkshop=value[0]
            localStorage.setItem('RepairWorkshopKey',this.ChoiceWorkshop)
        },
        changeProcess(value){
            this.ChoiceProcess=value[0]
            localStorage.setItem('RepairProcessKey',this.ChoiceProcess)
        },
        changeResource(value){
            this.ChoiceResource=value[0]
            localStorage.setItem('RepairResourceKey',this.ChoiceResource)
        }
    },
    mounted () {
        ...
        if (localStorage.getItem('RepairWorkshopKey')) {
            let newArr=this.WorkshopList[0].filter(item=>{
                return item==localStorage.getItem('RepairWorkshopKey')
            })
            if(newArr.length>0){
                this.ChoiceWorkshop=localStorage.getItem('RepairWorkshopKey')
            }
        }

        //获取存在本地的工序，并且判断之前存储的值是否在当前工序列表中存在，存在才赋值
        if (localStorage.getItem('RepairProcessKey')) {
            let newArr=this.ProcessList[0].filter(item=>{
                return item==localStorage.getItem('RepairProcessKey')
            })
            console.log(newArr);
            if(newArr.length>0){
                console.log(666);
                this.ChoiceProcess=localStorage.getItem('RepairProcessKey')
            }
        }
        //获取存在本地的资源，并且判断之前存储的值是否在当前资源列表中存在，存在才赋值
        if (localStorage.getItem('RepairResourceKey')) {
            let newArr=this.ResourceList[0].filter(item=>{
                return item==localStorage.getItem('RepairResourceKey')
            })
            if(newArr.length>0){
                this.ChoiceResource=localStorage.getItem('RepairResourceKey')
            }
        }
