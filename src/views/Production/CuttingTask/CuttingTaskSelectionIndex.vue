...
export default {
  ...
  methods: {
    ...
    changeWorkshop(value) {
      ...
      localStorage.setItem("CuttingTaskWorkshopKey", this.ChoiceWorkshopId);
    ...
    changeProcess(value) {
      ...
      localStorage.setItem("CuttingTaskProcessKey", this.ChoiceProcessId);
    ...
    changeTeamGroup(value) {
      ...
      localStorage.setItem("CuttingTaskResourceKey", this.ChoiceTeamGroupId);
      this.ChoiceResource = this.GetResource.find((item) => item.Id == id).Name;
    },
    //接口：获取车间数据接口
