...
export default {
    ...
    data() {
        return {
            ...
            ChoiceWorkshop:null,        //选择的车间code
            ChoiceWorkshopId:null,      //选择的车间id
            WorkshopName:null,          //选择的车间名称
    ...
    methods: {
        ...
        goCollection(){
            ...
            //这里不需要做考勤验证
            this.$router.push({name:'CheckWorkCollection',params:{
        ...
        changeWorkshop(value){
            ...
            localStorage.setItem('CheckWorkWorkshopKey',this.ChoiceWorkshopId)
            this.ChoiceWorkshop = this.GetWorkshop.find(item=>item.Id == id).Code
            this.WorkshopName = this.GetWorkshop.find(item=>item.Id == id).Name
        ...
        changeProcess(value){
            ...
            localStorage.setItem('CheckWorkProcessKey',this.ChoiceProcessId)
            this.ChoiceProcess = this.GetProcess.find(item=>item.Id == id).Name
            this.ChoiceResourceId=null
            this.ChoiceResource=null
            localStorage.setItem('CheckWorkResourceKey',this.ChoiceResourceId)
        },
        //选择某一项资源
        ...
        async judgeWorkshop(){
            ...
            if (localStorage.getItem('CheckWorkWorkshopKey')) {
                let newArr=this.GetWorkshop.filter(item=>{
                    return item.Id==localStorage.getItem('CheckWorkWorkshopKey')
                })
                if(newArr.length>0){
                    this.ChoiceWorkshopId=localStorage.getItem('CheckWorkWorkshopKey')
                    this.ChoiceWorkshop=newArr[0].Code
                    this.WorkshopName = newArr[0].Name
                }
            }
        },
        ...
        async judgeProcess(){
            ...
            if (localStorage.getItem('CheckWorkProcessKey')) {
                let newArr=this.GetProcess.filter(item=>{
                    return item.Id==localStorage.getItem('CheckWorkProcessKey')
                })
                if(newArr.length>0){
                    this.ChoiceProcessId=localStorage.getItem('CheckWorkProcessKey')
        ...
        async judgeResources(){
            ...
            if (localStorage.getItem('CheckWorkResourceKey')) {
                let newArr=this.GetResource.filter(item=>{
                    return item.Id==localStorage.getItem('CheckWorkResourceKey')
                })
                if(newArr.length>0){
                    this.ChoiceResourceId=localStorage.getItem('CheckWorkResourceKey')
