  ...
  <div class="g-index">
    ...
    <div class="f-flexvw f-flexg1" style="margin-top: 30px">
      <div class="g-scrollbox">
        ...
        <div class="m-inp f-mtb5">
          <span class="laber">异常类型</span>
          <div @click="doShowExceptionType" class="m-selector">
            <popup-picker
              :show.sync="ShowExceptionType"
              :data="ExceptionTypeList"
              @on-change="changeExceptionType"
              value-text-align="left"
            ></popup-picker>
            <div>{{ ChoiceExceptionType }}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      ShowExceptionType: false, //控制异常类型弹窗的显隐
      ChoiceExceptionType: null, //选择的异常类型名称
      ChoiceExceptionTypeId: null, //选择的异常类型id
      ExceptionTypeList: [[" "]], //显示的异常类型列表
      GetExceptionType: null, //接口获取异常类型的数据
    };
  },
  components: {},
  methods: {
    ...
    goDetail() {
      ...
      if (this.ChoiceExceptionType == "" || this.ChoiceExceptionType == null) {
        this.Msg = "请先选择异常类型";
        this.showPositionValue = true;
        return;
      }
      this.$router.push({
        ...
        params: {
          ...
          WorkshopName: this.ChoiceWorkshop,
          ExceptionType: this.ChoiceExceptionType,
        },
      });
    ...
    changeExceptionType(value) {
      let code = value[0];
      this.ChoiceExceptionType = this.GetExceptionType.find((item) => item.Code == code).Name;
    },
    //接口：获取车间数据接口
    ...
    async judgeWorkshop() {
      ...
      if (this.SelectWorkShopId != null) {
        ...
        if (localStorage.getItem("CuttingTaskWorkshopKey")) {
          let newArr = this.GetWorkshop.filter((item) => {
            return item.Id == localStorage.getItem("CuttingTaskWorkshopKey");
          });
          if (newArr.length > 0) {
            this.ChoiceWorkshopId = localStorage.getItem(
              "CuttingTaskWorkshopKey"
            );
            this.ChoiceWorkshop = newArr[0].Code;
          }
        }
    ...
    async judgeProcess() {
      ...
      if (this.SelectProcessId != null) {
        ...
        if (localStorage.getItem("CuttingTaskProcessKey")) {
          let newArr = this.GetProcess.filter((item) => {
    ...
    async judgeResources() {
      ...
      if (localStorage.getItem("CuttingTaskResourceKey")) {
    ...
    doShowExceptionType() {
      this.ShowExceptionType = true;
      this.$axiosApi.getExceptionTypeList().then((res) => {
        if (res.Success) {
          this.GetExceptionType = res.Result;
          this.ExceptionTypeList = [
            this.GetExceptionType.map((item) => {
