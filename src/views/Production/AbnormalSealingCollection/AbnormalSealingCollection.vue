    ...
    <div class="f-flexvw f-flexg1 f-pdlr5">
      <div class="g-inp" style="min-height: 174px">
        ...
        <div class="m-inp f-mtb5">
          ...
          <span class="inp s-inpbg">
            <input
              v-model="NewSupportNumber"
              ref="RefNewSupportNumber"
              :disabled="DataList.length >= 1"
              placeholder="请扫描托号"
              type="text"
              @keyup.enter="getNewSupportNumber()"
              class="s-inpbg"
              @focus="getFocus()"
              @blur="getBlur()"
            />
            <span
              class="iconfont icon-iconfontscan"
              @click="scanBarCodeByNewSupportNumber"
            ></span>
          </span>
        </div>
        ...
        <div class="m-selectinp f-mtb5" style="justify-content: space-around">
          ...
          <!-- <x-switch class="f-mtb5" style="font-size: 16px;padding:0 10px;" title="" :value-map="['2','4']" v-model="Edge" @on-change="changeEdge"></x-switch> -->
          <s-switch
            @changeSwitch="changeTransformation"
            :inputValue="transformation"
          ></s-switch>
          <div style="font-size: 16px; margin-left: 5px; visibility: hidden">
            占位
          </div>
        </div>
      </div>
  ...
  methods: {
    scanBarCode() {
      ...
      this.$hybridApi.scanCode({
        success: function (res) {
          that.PanelNumber = res;
          that.getSupportNumber();
        },
      });
    },
    scanBarCodeByNewSupportNumber() {
      let that = this;
      console.log("调用扫描接口");
      this.$hybridApi.scanCode({
        success: function (res) {
          that.NewSupportNumber = res;
          that.getNewSupportNumber();
        },
      });
    },
    // 切换switch的事件
    ...
    getSupportNumber() {
      ...
      if (!!this.PanelNumber && !!this.NewSupportNumber) {
        this.doSacnPlateOnIgnoreSourcePlateCodePda(
          this.PanelNumber,
    ...
    doSacnPlateOnIgnoreSourcePlateCodePda(
      OldPlateUpiCode,
      EndPlateCode,
      CurrentProcessId
    ) {
    ...
    doSubmitFbChangeOnIgnoreSourcePlateCodePda(
      OldPlateCode,
      DepartmentCode,
      ResourceManageId,
      CurrentProcessId,
